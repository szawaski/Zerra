...
namespace Zerra.Repository.MySql
{
    public sealed partial class MySqlEngine : ITransactStoreEngine
    {
        ...
        private void AssureTable(ModelDetail model)
        {
            ...
            //MySql only allows AutoGenerated as the single primary key
            if (identityColumns.Length > 0 && !identityColumns.Any(x => x.IsIdentityAutoGenerated))
            {
...
AND C.TABLE_SCHEMA != 'world'
AND C.TABLE_SCHEMA != 'sakila'";
