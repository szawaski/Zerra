...
{
    public sealed class MySqlEngine : ISqlEngine
    {
        ...
        {
            return LinqMySqlConverter.Convert(select, where, order, skip, take, graph, modelDetail);
        }
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static CoreTypeSetter<TModel>[] ReadColumns<TModel>(DbDataReader reader, ModelDetail modelDetail)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static TModel ReadRow<TModel>(DbDataReader reader, ModelDetail modelDetail, CoreTypeSetter<TModel>[] columnProperties)
        {
        ...
        public void AssureDataStore(ICollection<ModelDetail> modelDetails)
        {
            ...
            {
                Log.ErrorAsync($"{nameof(ISqlEngine.AssureDataStore)} {nameof(MySqlEngine)} error while assuring datastore.", ex);
                throw;
            }
        }
                ...
                else
                {
                    ...
                    if (!sameType)
                    {
                        if (sqlColumn.IsPrimaryKey || sqlColumn.IsIdentity || sqlColumn.IsPrimaryKey != column.IsIdentity || sqlColumn.IsIdentity != column.IsIdentityAutoGenerated)
                            throw new Exception($"{nameof(ISqlEngine.AssureDataStore)} {nameof(MySqlEngine)} cannot automatically change column with a Primary Key or Identity {model.Type.GetNiceName()}.{column.Name}");
                ...
                if (column == null)
                {
                    ...
                    if (!sqlColumn.IsNullable)
                    {
                        if (sqlColumn.IsPrimaryKey || sqlColumn.IsIdentity)
                            throw new Exception($"{nameof(ISqlEngine.AssureDataStore)} {nameof(MySqlEngine)} needs to make {sqlColumn} nullable but cannot automatically change column with a Primary Key or Identity");
        ...
        private static void AssureConstraints(ModelDetail model, SqlConstraint[] sqlConstraints)
        {
            ...
            foreach (var columnForRelation in model.RelatedNonEnumerableProperties)
            {
                ...
                if (relatedModelDetails.IdentityProperties.Count != 1)
                    throw new Exception($"{nameof(ISqlEngine.AssureDataStore)} {nameof(MySqlEngine)} does not support automatic constraints with multiple identities {relatedModelDetails.Type.GetNiceName()}");
        ...
        private static void WriteSqlTypeFromModel(StringBuilder sb, ModelPropertyDetail property)
        {
            ...
            {
                if (!canBeIdentity)
                    throw new Exception($"{nameof(ISqlEngine.AssureDataStore)} {nameof(MySqlEngine)} {property.Type.GetNiceName()} {property.Name} cannot be an auto generated identity");
        ...
        private static void WriteSqlTypeFromColumnAsNullable(StringBuilder sb, SqlColumnType sqlColumn)
        {
            switch (sqlColumn.DataType)
            {
                ...
                case "varbinary":
                    ...
                    break;
                default:
                    throw new NotImplementedException($"{nameof(ISqlEngine.AssureDataStore)} {nameof(MySqlEngine)} type {sqlColumn.DataType} not implemented.");
            }
        ...
        private static bool ModelMatchesSqlType(ModelPropertyDetail property, SqlColumnType sqlColumn)
        {
            ...
            throw new Exception($"{nameof(ISqlEngine.AssureDataStore)} cannot match type {property.Type.GetNiceName()} to an {nameof(MySqlEngine)} type.");
        }
