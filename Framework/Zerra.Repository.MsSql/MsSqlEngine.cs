...
namespace Zerra.Repository.MsSql
{
    ...
    public sealed partial class MsSqlEngine : ITransactStoreEngine
    {
        ...
        private static ModelMemberDetail[] ReadColumns<TModel>(SqlDataReader reader, ModelDetail modelDetail)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static TModel ReadRow<TModel>(SqlDataReader reader, ModelDetail modelDetail, ModelMemberDetail[] columnProperties)
        {
        ...
        private static string GenerateSqlInsert<TModel>(TModel model, Graph? graph, ModelDetail modelDetail, bool getIdentities) where TModel : class, new()
        {
            ...
            try
            {
                ...
                if (getIdentities)
                {
                    var sbOutput = new CharWriter();
                    try
                    {
                        foreach (var modelPropertyDetail in modelDetail.IdentityAutoGeneratedMembers)
                        {
                            var isProperty = graph is null || graph.HasMember(modelPropertyDetail.Name);
                            if (!modelPropertyDetail.IsNullable || isProperty)
                            {
                                ...
                                if (value is not null)
                                {
                                    ...
                                    if (sbOutput.Length > 0)
                                        sbOutput.Write(',');
                                    sbOutput.Write("INSERTED.");
                                    sbOutput.Write(property);
                                }
                            }
                        }

                        if (sbColumns.Length > 0 && sbValues.Length > 0)
                            sql = $"INSERT INTO [{modelDetail.DataSourceEntityName}] ({sbColumns.ToString()}) OUTPUT ({sbOutput.ToString()}) VALUES ({sbValues.ToString()})\r\n SELECT SCOPE_IDENTITY()";
                        else
                            sql = $"INSERT INTO [{modelDetail.DataSourceEntityName}] OUTPUT ({sbOutput.ToString()}) DEFAULT VALUES";
                    }
                    finally
                    {
                        sbOutput.Dispose();
                ...
                switch (property.CoreType.Value)
                {
                    ...
                    case CoreType.DateTimeNullable:
                        if (property.DatePart == StoreDatePart.Date)
                            _ = sb.Append("[date]");
                        else if (property.DataSourcePrecisionLength.HasValue)
                            _ = sb.Append("[datetime2](").Append(property.DataSourcePrecisionLength.Value).Append(')');
                        else
        ...
        private static bool ModelMatchesSqlType(ModelMemberDetail property, SqlColumnType sqlColumn)
        {
            ...
            if (property.CoreType.HasValue)
            {
                switch (property.CoreType.Value)
                {
                    ...
                    case CoreType.DateTime: return ((sqlColumn.DataType == "datetime" && property.DatePart == StoreDatePart.DateTime && !property.DataSourcePrecisionLength.HasValue) || (sqlColumn.DataType == "datetime2" && property.DatePart == StoreDatePart.DateTime && sqlColumn.DatetimePrecision == property.DataSourcePrecisionLength) || (sqlColumn.DataType == "date" && property.DatePart == StoreDatePart.Date)) && sqlColumn.IsNullable == false;
                    ...
                    case CoreType.DateTimeNullable: return ((sqlColumn.DataType == "datetime" && property.DatePart == StoreDatePart.DateTime && !property.DataSourcePrecisionLength.HasValue) || (sqlColumn.DataType == "datetime2" && property.DatePart == StoreDatePart.DateTime && sqlColumn.DatetimePrecision == property.DataSourcePrecisionLength) || (sqlColumn.DataType == "date" && property.DatePart == StoreDatePart.Date)) && sqlColumn.IsNullable == true;
	...
	,CAST(CASE WHEN COLUMNPROPERTY(OBJECT_ID(C.TABLE_SCHEMA + '.' + C.TABLE_NAME), C.COLUMN_NAME, 'IsIdentity') = 1 OR COLUMN_DEFAULT = '(newid())' THEN 1 ELSE 0 END AS bit) AS IS_AUTOGENERATED
