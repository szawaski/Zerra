...
{
    /// <summary>
    /// Represents a generation plan for building or updating a MariaDb data store schema.
    /// </summary>
    public sealed class MariaDbDataStoreGenerationPlan : IDataStoreGenerationPlan
    {
        private readonly MariaDbEngine engine;
        ...
        /// <summary>
        /// Initializes a new instance of <see cref="MariaDbDataStoreGenerationPlan"/>.
        /// </summary>
        /// <param name="engine">The <see cref="MariaDbEngine"/> used to execute the plan.</param>
        ...
        public MariaDbDataStoreGenerationPlan(MariaDbEngine engine, string? createDatabaseName, ICollection<string> sql)
        {
            this.engine = engine;
        ...
        public void Execute(ILogger? log)
        {
            if (!String.IsNullOrWhiteSpace(createDatabaseName))
            {
                try
                {
                    ...
                    log?.Info($"{nameof(MariaDbEngine)}: Create Database {createDatabaseName}");
                }
                catch (Exception ex)
                {
                    log?.Error($"{nameof(MariaDbEngine)} error while creating datastore.", ex);
                }
            }
            ...
            foreach (var line in sql)
            {
                try
                {
                    ...
                    log?.Info($"{nameof(MariaDbEngine)}: {line}");
                }
                catch (Exception ex)
                {
                    log?.Error($"{nameof(MariaDbEngine)} error while assuring datastore: {line}.", ex);
                    throw;
                }
            }
        }
