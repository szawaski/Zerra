...
namespace Zerra.Logger
{
    public sealed class BusLoggingProvider : IBusLogger
    {
        ...
        public void BeginCommand(Type commandType, ICommand command, string source, bool handled)
        {
        ...
        public void BeginEvent(Type eventType, IEvent @event, string source, bool handled)
        {
        ...
        public void EndCommand(Type commandType, ICommand command, string source, bool handled, long milliseconds, Exception? ex)
        {
            var typeName = commandType.GetNiceName();
            var user = System.Threading.Thread.CurrentPrincipal?.Identity?.Name ?? anonymous;
        ...
        public void EndEvent(Type eventType, IEvent @event, string source, bool handled, long milliseconds, Exception? ex)
        {
            var typeName = eventType.GetNiceName();
            var user = System.Threading.Thread.CurrentPrincipal?.Identity?.Name ?? anonymous;
        ...
        public void EndCall(Type interfaceType, string methodName, object[] arguments, object? result, string source, bool handled, long milliseconds, Exception? ex)
        {
            var interfaceName = interfaceType.GetNiceName();
            var user = System.Threading.Thread.CurrentPrincipal?.Identity?.Name ?? anonymous;
