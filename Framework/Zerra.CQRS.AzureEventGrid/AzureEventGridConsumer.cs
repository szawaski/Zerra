...
{
    public partial class AzureEventGridConsumer : ICommandConsumer, IEventConsumer, IDisposable
    {
        ...
        public AzureEventGridConsumer(string host, SymmetricKey encryptionKey)
        {
            this.host = host;
        ...
        {
            Open();
            _ = Log.InfoAsync($"{nameof(AzureEventGridConsumer)} Command Server Started Connected To {this.host}");
        }
        void IEventConsumer.Open()
        {
            Open();
            _ = Log.InfoAsync($"{nameof(AzureEventGridConsumer)} Event Server Started Connected To {this.host}");
        }
        private void Open()
        {
            isOpen = true;
        ...
        {
            Close();
            _ = Log.InfoAsync($"{nameof(AzureEventGridConsumer)} Command Server Closed On {this.host}");
        }
        void IEventConsumer.Close()
        {
            Close();
            _ = Log.InfoAsync($"{nameof(AzureEventGridConsumer)} Event Server Closed On {this.host}");
        }
        private void Close()
        {
            if (isOpen)
