...
namespace Zerra.Repository.Reflection
{
    public static class CoreTypeSetterGenerator
    {
        ...
        {
            var generatedType = generatedTypes.GetOrAdd(memberInfo, coreType, isByteArray, static (memberInfo, coreType, isByteArray) => Generate(memberInfo, coreType, isByteArray));
        ...
        {
            var interfaceType = GenericTypeCache.GetGenericType(iCoreTypeSetterType, memberInfo.ReflectedType!);
            ...
            foreach (var method in methods)
            {
                ...
                if (TypeLookup.GetCoreType(parameterTypes[1], out var coreTypeLookup))
                {
