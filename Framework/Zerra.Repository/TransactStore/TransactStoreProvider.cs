        ...
        where TModel : class, new()
    {
        ...
        protected override sealed void PersistModel(PersistEvent @event, object model, Graph? graph, bool create)
        {
            if (create)
            {
                ...
                {
                    var identities = (IList<object>)Engine.ExecuteInsertGetIdentities(model, graph, ModelTypeDetail);
                    var i = 0;
                ...
                {
                    var success = Engine.ExecuteInsert(model, graph, ModelTypeDetail);
                    if (!success)
            ...
            {
                var success = Engine.ExecuteUpdate(model, graph, ModelTypeDetail);
                if (!success)
                    throw new Exception($"No rows affected: {(String.IsNullOrWhiteSpace(@event.Name) ? defaultEventName : @event.Name)}");
            }
        }
        ...
        protected override sealed async Task PersistModelAsync(PersistEvent @event, object model, Graph? graph, bool create)
        {
            if (create)
            {
                ...
                {
                    var identities = (IList<object>)await Engine.ExecuteInsertGetIdentitiesAsync(model, graph, ModelTypeDetail);
                    var i = 0;
                    foreach (var modelPropertyInfo in ModelTypeDetail.IdentityAutoGeneratedProperties)
                    {
                        var identity = modelPropertyInfo.GetterBoxed(identities[i])!;
                ...
                {
                    var success = await Engine.ExecuteInsertAsync(model, graph, ModelTypeDetail);
                    if (!success)
                        throw new Exception($"No rows affected: {(String.IsNullOrWhiteSpace(@event.Name) ? defaultEventName : @event.Name)}");
                }
            }
            ...
            {
                var success = await Engine.ExecuteUpdateAsync(model, graph, ModelTypeDetail);
                if (!success)
                    throw new Exception($"No rows affected: {(String.IsNullOrWhiteSpace(@event.Name) ? defaultEventName : @event.Name)}");
            }
        }
