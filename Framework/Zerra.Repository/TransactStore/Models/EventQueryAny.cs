...
{
    /// <summary>
    /// A query against the event store that checks whether any <typeparamref name="TModel"/> event records
    /// exist within the specified date or event-number range.
    /// </summary>
    ...
    public sealed class EventQueryAny<TModel> : Query<TModel> where TModel : class, new()
    {
        ...
        /// <summary>Initializes a new instance scoped to a date range with a sort order; filter defaults to <see langword="null"/>.</summary>
        ...
        /// <summary>Initializes a new instance scoped to an event-number range with a sort order; filter defaults to <see langword="null"/>.</summary>
