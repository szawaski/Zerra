...
{
    public static class TransactStoreGenerator
    {
        public static void Generate(SourceProductionContext context, string ns, StringBuilder sbInitializer, ITypeSymbol symbol)
        {
            ...
            {
                if (attribute.AttributeClass?.Name != "TransactStoreEntityAttribute")
                    return;
                
                var modelSymbol = attribute.AttributeClass.TypeArguments[0];
                var typeNameForClass = Helpers.GetNameForClass(modelSymbol);
                var className = $"TransactStoreProvider_{typeNameForClass}";
                ...
                {
                    public class {{className}} : Zerra.Repository.TransactStoreProvider<{{symbol.ToString()}}, {{modelSymbol.ToString()}}>
                    {
                        public {{className}}() { }
                    }
                }
