...
{
    public sealed class MsSqlEngine : ISqlEngine
    {
        ...
        {
            return LinqMsSqlConverter.Convert(select, where, order, skip, take, graph, modelDetail);
        }
                ...
                if (columnProperty != null)
                {
                    if (columnProperty.CoreType.HasValue)
                    {
                        switch (columnProperty.CoreType.Value)
                        {
                            ...
                            default:
                                throw new NotSupportedException($"MSSQL Cannot map to type {columnProperty.CoreType.Value} in {modelDetail.Type.GetNiceName()}");
                        }
                    ...
                    {
                        throw new NotSupportedException($"MSSQL cannot map to type in {modelDetail.Type.GetNiceName()}");
                    }
                }
            }
        ...
        //private static Action<object, object>[] ReadColumns<TModel>(SqlDataReader reader, ModelDetail modelDetail)
        //{
        //    var columnProperties = new Action<object, object>[reader.FieldCount];
        //    for (var i = 0; i < reader.FieldCount; i++)
        //    {
        //        var property = reader.GetName(i);
        //        if (modelDetail.TryGetProperty(property, out ModelPropertyDetail propertyInfo))
        //            columnProperties[i] = propertyInfo.Setter;
        //    }
        //    return columnProperties;
        //}
        //private static TModel ReadRow<TModel>(SqlDataReader reader, ModelDetail modelDetail, Action<object, object>[] columnProperties)
        //{
        //    var model = (TModel)modelDetail.Creator();
        ...
        public void AssureDataStore(ICollection<ModelDetail> modelDetails)
        {
            ...
            {
                Log.ErrorAsync($"{nameof(ISqlEngine.AssureDataStore)} MSSQL error while assuring datastore.", ex);
                throw;
            }
        }
                ...
                else
                {
                    ...
                    if (!sameType)
                    {
                        if (sqlColumn.IsPrimaryKey || sqlColumn.IsIdentity || sqlColumn.IsPrimaryKey != column.IsIdentity || sqlColumn.IsIdentity != column.IsIdentityAutoGenerated)
                            throw new Exception($"{nameof(ISqlEngine.AssureDataStore)} MSSQL cannot automatically change column with a Primary Key or Identity {model.Type.GetNiceName()}.{column.Name}");
                ...
                if (column == null)
                {
                    ...
                    if (!sqlColumn.IsNullable)
                    {
                        if (sqlColumn.IsPrimaryKey || sqlColumn.IsIdentity)
                            throw new Exception($"{nameof(ISqlEngine.AssureDataStore)} MSSQL needs to make {sqlColumn} nullable but cannot automatically change column with a Primary Key or Identity");
        ...
        private static void AssureConstraints(ModelDetail model, SqlConstraint[] sqlConstraints)
        {
            ...
            foreach (var columnForRelation in model.RelatedNonEnumerableProperties)
            {
                ...
                if (relatedModelDetails.IdentityProperties.Count != 1)
                    throw new Exception($"{nameof(ISqlEngine.AssureDataStore)} MSSQL does not support automatic constraints with multiple identities {relatedModelDetails.Type.GetNiceName()}");
        ...
        private static void WriteSqlTypeFromModel(StringBuilder sb, ModelPropertyDetail property)
        {
            ...
            if (property.CoreType.HasValue)
            {
                switch (property.CoreType.Value)
                {
                    ...
                    case CoreType.String:
                        ...
                        break;
                    default:
                        throw new Exception($"Cannot match type {property.Type.GetNiceName()} to an MSSQL type.");
                }
            }
            ...
            {
                if (!canBeIdentity)
                    throw new Exception($"{nameof(ISqlEngine.AssureDataStore)} MMSQL {property.Type.GetNiceName()} {property.Name} cannot be an auto generated identity");
        ...
        private static void WriteSqlTypeFromColumnAsNullable(StringBuilder sb, SqlColumnType sqlColumn)
        {
            switch (sqlColumn.DataType)
            {
                ...
                case "varbinary":
                    ...
                    break;
                default:
                    throw new NotImplementedException($"{nameof(ISqlEngine.AssureDataStore)} MSSQL type {sqlColumn.DataType} not implemented.");
            }
        ...
        private static bool ModelMatchesSqlType(ModelPropertyDetail property, SqlColumnType sqlColumn)
        {
            ...
            throw new Exception($"{nameof(ISqlEngine.AssureDataStore)} cannot match type {property.Type.GetNiceName()} to an MSSQL type.");
        }
        ...
        {
            var query = $@"
SELECT C.TABLE_NAME, COLUMN_NAME, DATA_TYPE, IS_NULLABLE, CHARACTER_MAXIMUM_LENGTH, NUMERIC_PRECISION, NUMERIC_SCALE, DATETIME_PRECISION
