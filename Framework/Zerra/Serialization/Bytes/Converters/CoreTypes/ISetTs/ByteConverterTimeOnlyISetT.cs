...
namespace Zerra.Serialization.Bytes.Converters.ISets
{
    internal sealed class ByteConverterTimeOnlyISet<TParent> : ByteConverter<TParent, ISet<TimeOnly>>
    {
        protected override sealed bool TryReadValue(ref ByteReader reader, ref ReadState state, bool nullFlags, out ISet<TimeOnly>? value)
        {
            ...
            if (!reader.TryRead(state.Current.EnumerableLength!.Value, out HashSet<TimeOnly>? valueTyped, out state.BytesNeeded))
            {
                state.Current.HasNullChecked = true;
                value = default;
                return false;
        ...
        protected override sealed bool TryWriteValue(ref ByteWriter writer, ref WriteState state, bool nullFlags, ISet<TimeOnly>? value)
        {
