...
{
    internal sealed class ByteConverterDoubleNullableIReadOnlyCollection<TParent> : ByteConverter<TParent, IReadOnlyCollection<double?>>
    {
        protected override sealed bool TryReadValue(ref ByteReader reader, ref ReadState state, bool nullFlags, out IReadOnlyCollection<double?>? value)
        {
            ...
            if (!reader.TryRead(state.Current.EnumerableLength!.Value, out List<double?>? valueTyped, out state.BytesNeeded))
            {
                state.Current.HasNullChecked = true;
                value = default;
                return false;
        ...
        protected override sealed bool TryWriteValue(ref ByteWriter writer, ref WriteState state, bool nullFlags, IReadOnlyCollection<double?>? value)
        {
