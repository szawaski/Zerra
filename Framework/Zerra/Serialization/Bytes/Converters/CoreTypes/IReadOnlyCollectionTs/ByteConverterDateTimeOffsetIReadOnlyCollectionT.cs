...
{
    internal sealed class ByteConverterDateTimeOffsetIReadOnlyCollection<TParent> : ByteConverter<TParent, IReadOnlyCollection<DateTimeOffset>>
    {
        protected override sealed bool TryReadValue(ref ByteReader reader, ref ReadState state, bool nullFlags, out IReadOnlyCollection<DateTimeOffset>? value)
        {
            ...
            if (!reader.TryRead(state.Current.EnumerableLength!.Value, out List<DateTimeOffset>? valueTyped, out state.BytesNeeded))
            {
                state.Current.HasNullChecked = true;
                value = default;
                return false;
        ...
        protected override sealed bool TryWriteValue(ref ByteWriter writer, ref WriteState state, bool nullFlags, IReadOnlyCollection<DateTimeOffset>? value)
        {
