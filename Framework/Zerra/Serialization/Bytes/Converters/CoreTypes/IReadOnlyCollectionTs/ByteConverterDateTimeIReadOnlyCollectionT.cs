...
{
    internal sealed class ByteConverterDateTimeIReadOnlyCollection<TParent> : ByteConverter<TParent, IReadOnlyCollection<DateTime>>
    {
        protected override sealed bool TryReadValue(ref ByteReader reader, ref ReadState state, bool nullFlags, out IReadOnlyCollection<DateTime>? value)
        {
            ...
            if (!reader.TryRead(state.Current.EnumerableLength!.Value, out List<DateTime>? valueTyped, out state.BytesNeeded))
            {
                state.Current.HasNullChecked = true;
                value = default;
                return false;
        ...
        protected override sealed bool TryWriteValue(ref ByteWriter writer, ref WriteState state, bool nullFlags, IReadOnlyCollection<DateTime>? value)
        {
