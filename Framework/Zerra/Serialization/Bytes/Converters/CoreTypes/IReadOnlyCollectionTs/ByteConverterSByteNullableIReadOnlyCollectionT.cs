...
{
    internal sealed class ByteConverterSByteNullableIReadOnlyCollection<TParent> : ByteConverter<TParent, IReadOnlyCollection<sbyte?>>
    {
        protected override sealed bool TryReadValue(ref ByteReader reader, ref ReadState state, bool nullFlags, out IReadOnlyCollection<sbyte?>? value)
        {
            ...
            if (!reader.TryRead(state.Current.EnumerableLength!.Value, out List<sbyte?>? valueTyped, out state.BytesNeeded))
            {
                state.Current.HasNullChecked = true;
                value = default;
                return false;
        ...
        protected override sealed bool TryWriteValue(ref ByteWriter writer, ref WriteState state, bool nullFlags, IReadOnlyCollection<sbyte?>? value)
        {
