...
{
    internal sealed class ByteConverterInt64IReadOnlyList<TParent> : ByteConverter<TParent, IReadOnlyList<long>>
    {
        protected override sealed bool TryReadValue(ref ByteReader reader, ref ReadState state, bool nullFlags, out IReadOnlyList<long>? value)
        {
            ...
            if (!reader.TryRead(state.Current.EnumerableLength!.Value, out List<long>? valueTyped, out state.BytesNeeded))
            {
                state.Current.HasNullChecked = true;
                value = default;
                return false;
        ...
        protected override sealed bool TryWriteValue(ref ByteWriter writer, ref WriteState state, bool nullFlags, IReadOnlyList<long>? value)
        {
