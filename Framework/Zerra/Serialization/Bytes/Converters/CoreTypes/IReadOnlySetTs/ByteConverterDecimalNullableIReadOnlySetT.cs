...
{
    internal sealed class ByteConverterDecimalNullableIReadOnlySet<TParent> : ByteConverter<TParent, IReadOnlySet<decimal?>>
    {
        protected override sealed bool TryReadValue(ref ByteReader reader, ref ReadState state, bool nullFlags, out IReadOnlySet<decimal?>? value)
        {
            ...
            if (!reader.TryRead(state.Current.EnumerableLength!.Value, out HashSet<decimal?>? valueTyped, out state.BytesNeeded))
            {
                state.Current.HasNullChecked = true;
                value = default;
                return false;
        ...
        protected override sealed bool TryWriteValue(ref ByteWriter writer, ref WriteState state, bool nullFlags, IReadOnlySet<decimal?>? value)
        {
