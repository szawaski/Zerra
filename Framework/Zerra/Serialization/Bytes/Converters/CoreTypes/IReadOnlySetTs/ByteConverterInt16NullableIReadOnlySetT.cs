...
{
    internal sealed class ByteConverterInt16NullableIReadOnlySet<TParent> : ByteConverter<TParent, IReadOnlySet<short?>>
    {
        protected override sealed bool TryReadValue(ref ByteReader reader, ref ReadState state, bool nullFlags, out IReadOnlySet<short?>? value)
        {
            ...
            if (!reader.TryRead(state.Current.EnumerableLength!.Value, out HashSet<short?>? valueTyped, out state.BytesNeeded))
            {
                state.Current.HasNullChecked = true;
                value = default;
                return false;
        ...
        protected override sealed bool TryWriteValue(ref ByteWriter writer, ref WriteState state, bool nullFlags, IReadOnlySet<short?>? value)
        {
