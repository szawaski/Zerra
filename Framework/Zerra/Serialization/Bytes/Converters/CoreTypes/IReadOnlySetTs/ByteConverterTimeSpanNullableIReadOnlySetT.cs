...
{
    internal sealed class ByteConverterTimeSpanNullableIReadOnlySet<TParent> : ByteConverter<TParent, IReadOnlySet<TimeSpan?>>
    {
        protected override sealed bool TryReadValue(ref ByteReader reader, ref ReadState state, bool nullFlags, out IReadOnlySet<TimeSpan?>? value)
        {
            ...
            if (!reader.TryRead(state.Current.EnumerableLength!.Value, out HashSet<TimeSpan?>? valueTyped, out state.BytesNeeded))
            {
                state.Current.HasNullChecked = true;
                value = default;
                return false;
        ...
        protected override sealed bool TryWriteValue(ref ByteWriter writer, ref WriteState state, bool nullFlags, IReadOnlySet<TimeSpan?>? value)
        {
