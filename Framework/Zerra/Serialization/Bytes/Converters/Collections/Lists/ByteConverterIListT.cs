...
{
    internal sealed class ByteConverterIListT<TParent, TValue> : ByteConverter<TParent, IList<TValue>>
    {
        ...
        protected override sealed void Setup()
        {
            ...
            readConverter = ByteConverterFactory<IList<TValue>>.Get(valueTypeDetail, nameof(ByteConverterIListT<TParent, TValue>), null, Setter);
            writeConverter = ByteConverterFactory<IEnumerator<TValue>>.Get(valueTypeDetail, nameof(ByteConverterIListT<TParent, TValue>), Getter, null);
        }

        protected override sealed bool TryReadValue(ref ByteReader reader, ref ReadState state, out IList<TValue>? value)
        {
            if (!state.Current.EnumerableLength.HasValue)
            {
                ...
                else
                {
                    ...
                    if (state.Current.EnumerableLength!.Value == 0)
                        return true;
                    value = new IListTCounter<TValue>();
                }
            }
            else
            {
        ...
        protected override sealed bool TryWriteValue(ref ByteWriter writer, ref WriteState state, in IList<TValue> value)
        {
