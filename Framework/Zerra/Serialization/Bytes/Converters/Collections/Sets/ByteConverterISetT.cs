...
namespace Zerra.Serialization.Bytes.Converters.Collections.Sets
{
    internal sealed class ByteConverterISetT<TParent, TValue> : ByteConverter<TParent, ISet<TValue>>
    {
        ...
        protected override sealed bool TryReadValue(ref ByteReader reader, ref ReadState state, bool nullFlags, out ISet<TValue>? value)
        {
            ...
            for (; ; )
            {
                var read = readConverter.TryReadFromParent(ref reader, ref state, value, true, false);
                if (!read)
                {
                    state.Current.HasNullChecked = true;
        ...
        protected override sealed bool TryWriteValue(ref ByteWriter writer, ref WriteState state, bool nullFlags, ISet<TValue>? value)
        {
