        ...
        protected override sealed bool TryReadValue(ref JsonReader reader, ref ReadState state, JsonToken token, out int? value)
        {
            switch (token)
            {
                case JsonToken.Number:
                    if (reader.UseBytes)
                    {
                        if ((!Utf8Parser.TryParse(reader.StringBytes, out int parsed, out var consumed) || consumed != reader.StringBytes.Length) && state.ErrorOnTypeMismatch)
                    ...
                    {
#if NETSTANDARD2_0
                        if (!Int32.TryParse(reader.ValueChars.ToString(), out int parsed) && state.ErrorOnTypeMismatch)
#else
                        if (!Int32.TryParse(reader.StringChars, out int parsed) && state.ErrorOnTypeMismatch)
#endif
                            ThrowCannotConvert(ref reader);
                        value = parsed;
                        return true;
                ...
                case JsonToken.String:
                    if (reader.UseBytes)
                    {
                        ...
                        if ((!Utf8Parser.TryParse(reader.StringBytes, out int parsed, out var consumed) || reader.StringBytes.Length != consumed) && state.ErrorOnTypeMismatch)
                            ThrowCannotConvert(ref reader);
                        value = parsed;
                        return true;
                    }
