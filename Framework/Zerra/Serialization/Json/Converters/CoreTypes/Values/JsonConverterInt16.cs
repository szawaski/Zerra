        ...
        protected override sealed bool TryReadValue(ref JsonReader reader, ref ReadState state, JsonToken token, out short value)
        {
            switch (token)
            {
                case JsonToken.Number:
                    ...
                    {
#if NETSTANDARD2_0
                        if (!Int16.TryParse(reader.ValueChars.ToString(), out value) && state.ErrorOnTypeMismatch)
#else
                        if (!Int16.TryParse(reader.StringChars, out value) && state.ErrorOnTypeMismatch)
#endif
                            ThrowCannotConvert(ref reader);
                        return true;
                    }
