            ...
            switch (valueType)
            {
                case JsonValueType.Number:
                    if (reader.UseBytes)
                    {
                        ...
                        if ((!Utf8Parser.TryParse(bytes, out decimal parsed, out var consumed) || consumed != bytes.Length) && state.ErrorOnTypeMismatch)
                ...
                case JsonValueType.String:
                    if (reader.UseBytes)
                    {
                        ...
                        if ((!Utf8Parser.TryParse(bytes, out decimal parsed, out var consumed) || bytes.Length != consumed) && state.ErrorOnTypeMismatch)
                            ThrowCannotConvert(ref reader);
                        value = parsed;
                        return true;
                    }
