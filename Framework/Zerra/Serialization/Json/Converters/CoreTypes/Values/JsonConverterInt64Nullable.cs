            ...
            switch (valueType)
            {
                case JsonValueType.Number:
                    if (reader.UseBytes)
                    {
                        ...
                        if ((!Utf8Parser.TryParse(bytes, out long parsed, out var consumed) || consumed != bytes.Length) && state.ErrorOnTypeMismatch)
...
#if NETSTANDARD2_0
                        if (!Int64.TryParse(chars.ToString(), out long parsed) && state.ErrorOnTypeMismatch)
#else
                        if (!Int64.TryParse(chars, out long parsed) && state.ErrorOnTypeMismatch)
#endif
                            ThrowCannotConvert(ref reader);
                        value = parsed;
                        return true;
                ...
                case JsonValueType.String:
                    if (reader.UseBytes)
                    {
                        ...
                        if ((!Utf8Parser.TryParse(bytes, out long parsed, out var consumed) || bytes.Length != consumed) && state.ErrorOnTypeMismatch)
                            ThrowCannotConvert(ref reader);
                        value = parsed;
                        return true;
                    }
