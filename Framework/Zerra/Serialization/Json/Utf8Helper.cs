    ...
    internal static partial class Utf8Helper
    {
        ...
        public unsafe static bool TryParse(ReadOnlySpan<byte> source, out DateTimeOffset value)
        {
                ...
                if (afterTime == dotByte)
                {
                    for (; i < 27 && i < length; i++)
                    {
                        ...
                        if (d1 == zUpperByte)
                        {
                            ...
                            for (; fractionDigits < 7; fractionDigits++)
                                fraction *= 10;
                            value = CreateDateTimeOffset(year, month, day, hours, minutes, seconds, fraction, 0, 0);
                            return true;
                        }
                        if (d1 == plusByte)
                        {
            ...
            timezone:
                ...
                value = CreateDateTimeOffset(year, month, day, hours, minutes, seconds, fraction, offsetHours, offsetMinutes);
                return true;
            }
        }
        ...
        private static DateTimeOffset CreateDateTimeOffset(int year, int month, int day, int hours, int minutes, int seconds, int fraction, int offsetHours, int offsetMinutes)
        {
            ...
            var ticks = (totalDays * TimeSpan.TicksPerDay) + (((hours * 3600) + (minutes * 60) + seconds) * TimeSpan.TicksPerSecond) + fraction;
