...
namespace Zerra.Map
{
    public static class InterpretedMapGenerator
    {
        ...
        private static TTarget Map<TSource, TTarget>(TSource source, TTarget? target, Graph? graph)
            where TSource : notnull
            where TTarget : notnull
            => (TTarget)Map(source!, target, graph, typeof(TSource).GetTypeDetail(), typeof(TTarget).GetTypeDetail());

        private static object Map(object source, object? target, Graph? graph, TypeDetail sourceType, TypeDetail targetType)
        {
            if (source == null)
