...
{
    public sealed partial class BusInstance : IDisposable, IAsyncDisposable
    {
        ...
        private static readonly MethodDetail sendMethodLoggedGenericAsyncMethod = typeof(BusInstance).GetMethodDetail(nameof(SendMethodLoggedGenericAsync));
        private static readonly MethodDetail callInternalLoggedGenericAsyncMethod = typeof(BusInstance).GetMethodDetail(nameof(CallMethodInternalLoggedGenericAsync));
        ...
        private SemaphoreSlim? processWaiter = null;
        private int maxConcurrentQueries = Environment.ProcessorCount * 32;
        private int maxConcurrentCommandsPerTopic = Environment.ProcessorCount * 8;
        private int maxConcurrentEventsPerTopic = Environment.ProcessorCount * 16;
        private CommandCounter commandCounter = new();

        private readonly ConcurrentDictionary<Type, ICommandProducer> commandProducers = new();
        private readonly ConcurrentReadWriteHashSet<ICommandConsumer> commandConsumers = new();
        private readonly HashSet<Type> handledCommandTypes = new();
        private readonly ConcurrentDictionary<Type, IEventProducer> eventProducers = new();
        private readonly ConcurrentReadWriteHashSet<IEventConsumer> eventConsumers = new();
        private readonly HashSet<Type> handledEventTypes = new();
        private readonly ConcurrentDictionary<Type, IQueryClient> queryClients = new();
        private readonly ConcurrentReadWriteHashSet<IQueryServer> queryServers = new();
        private readonly HashSet<Type> handledQueryTypes = new();
        private IBusLogger? busLogger = null;

        private readonly BusRouters busRouters;
        public BusInstance()
        {
            busRouters = new(this);
        }

        public async Task<RemoteQueryCallResponse> HandleRemoteQueryCallAsync(Type interfaceType, string methodName, string[] arguments, string source, bool isApi)
        {
        ...
        public Task HandleRemoteCommandDispatchAsync(ICommand command, string source, bool isApi)
        {
            var networkType = isApi ? NetworkType.Api : NetworkType.Internal;
            return DispatchCommandInternalAsync(command, false, networkType, source);
        }
        public Task HandleRemoteCommandDispatchAwaitAsync(ICommand command, string source, bool isApi)
        {
            var networkType = isApi ? NetworkType.Api : NetworkType.Internal;
            return DispatchCommandInternalAsync(command, true, networkType, source);
        }
        public Task HandleRemoteEventDispatchAsync(IEvent @event, string source, bool isApi)
        {
            var networkType = isApi ? NetworkType.Api : NetworkType.Internal;
            return DispatchEventInternalAsync(@event, networkType, source);
        }

        public Task DispatchAsync(ICommand command) => DispatchCommandInternalAsync(command, false, NetworkType.Local, Config.ApplicationIdentifier);
        public Task DispatchAwaitAsync(ICommand command) => DispatchCommandInternalAsync(command, true, NetworkType.Local, Config.ApplicationIdentifier);
        public Task DispatchAsync(IEvent @event) => DispatchEventInternalAsync(@event, NetworkType.Local, Config.ApplicationIdentifier);

        private Task DispatchCommandInternalAsync(ICommand command, bool requireAffirmation, NetworkType networkType, string source)
        {
        ...
        private Task DispatchEventInternalAsync(IEvent @event, NetworkType networkType, string source)
        {
        ...
        public Task _DispatchCommandInternalAsync(ICommand command, Type commandType, bool requireAffirmation, NetworkType networkType, string source, BusLogging busLogging)
        {
        ...
        public Task _DispatchEventInternalAsync(IEvent @event, Type eventType, NetworkType networkType, string source, BusLogging busLogging)
        {
        ...
        private async Task SendCommandLoggedAsync(ICommand command, Type commandType, bool requireAffirmation, NetworkType networkType, string source, ICommandProducer producer)
        {
        ...
        private async Task SendEventLoggedAsync(IEvent @event, Type eventType, NetworkType networkType, string source, IEventProducer producer)
        {
        ...
        private Task HandleCommandAsync(ICommand command, Type commandType, string source)
        {
        ...
        private async Task HandleCommandLoggedAsync(ICommand command, Type commandType, string source)
        {
            var interfaceType = TypeAnalyzer.GetGenericType(iCommandHandlerType, commandType);
        ...
        private Task HandleEventAsync(IEvent @event, Type eventType, string source)
        {
        ...
        private async Task HandleEventLoggedAsync(IEvent @event, Type eventType, string source)
        {
            var interfaceType = TypeAnalyzer.GetGenericType(iEventHandlerType, eventType);
        ...
        public TInterface Call<TInterface>() => (TInterface)CallInternal(typeof(TInterface), NetworkType.Local, Config.ApplicationIdentifier);
        public object Call(Type interfaceType) => CallInternal(interfaceType, NetworkType.Local, Config.ApplicationIdentifier);

        private object CallInternal(Type interfaceType, NetworkType networkType, string source)
        {
        ...
        public TReturn _CallMethod<TReturn>(Type interfaceType, string methodName, object[] arguments, NetworkType networkType, string source)
        {
            var methodDetail = TypeAnalyzer.GetMethodDetail(interfaceType, methodName);
            ...
            if (!queryClients.IsEmpty && queryClients.TryGetValue(interfaceType, out var methodCaller))
            {
                ...
                else if (methodDetail.ReturnType.IsTask)
                {
                    if (methodDetail.ReturnType.Type.IsGenericType)
                    {
                        ...
                        result = method.Caller(this, new object[] { interfaceType, methodName, arguments, networkType, source, methodDetail, methodCaller });
                    }
                    else
                    {
            ...
            else
            {
                ...
                else if (methodDetail.ReturnType.IsTask)
                {
                    if (methodDetail.ReturnType.Type.IsGenericType)
                    {
                        ...
                        result = method.Caller(this, new object[] { interfaceType, methodName, arguments, source, methodDetail });
                    }
                    else
                    {
        ...
        private async Task<TReturn> SendMethodLoggedGenericAsync<TReturn>(Type interfaceType, string methodName, object[] arguments, NetworkType networkType, string source, MethodDetail methodDetail, IQueryClient methodCaller)
        {
            busLogger?.BeginCall(interfaceType, methodName, arguments, null, source, false);
        ...
        private async Task SendMethodLoggedAsync<TReturn>(Type interfaceType, string methodName, object[] arguments, NetworkType networkType, string source, MethodDetail methodDetail, IQueryClient methodCaller)
        {
            busLogger?.BeginCall(interfaceType, methodName, arguments, null, source, false);
        ...
        private async Task<TReturn> CallMethodInternalLoggedGenericAsync<TReturn>(Type interfaceType, string methodName, object[] arguments, string source, MethodDetail methodDetail)
        {
            var providerType = ProviderResolver.GetTypeFirst(interfaceType);
            var provider = Instantiator.GetSingle(providerType);
        ...
        private async Task CallMethodInternalLoggedAsync(Type interfaceType, string methodName, object[] arguments, string source, MethodDetail methodDetail)
        {
            var providerType = ProviderResolver.GetTypeFirst(interfaceType);
            var provider = Instantiator.GetSingle(providerType);
        ...
        private HashSet<Type> GetCommandTypesFromInterface(Type interfaceType)
        {
        ...
        private HashSet<Type> GetEventTypesFromInterface(Type interfaceType)
        {
            var messageTypes = new HashSet<Type>();
            var typeDetail = TypeAnalyzer.GetTypeDetail(interfaceType);
        ...
        private string GetCommandTopic(Type commandType)
        {
            var interfaceType = TypeAnalyzer.GetGenericType(iCommandHandlerType, commandType);
            for (; ; )
            {
        ...
        private string GetEventTopic(Type eventType)
        {
            var interfaceType = TypeAnalyzer.GetGenericType(iEventHandlerType, eventType);
            for (; ; )
            {
        ...
        public void AddQueryServer(IQueryServer queryServer)
        {
            setupLock.Wait();
            try
            {
        ...
        public void AddLogger(IBusLogger busLogger)
        {
            setupLock.Wait();
            try
            {
                if (this.busLogger != null)
        ...
        private void Authenticate(IPrincipal? principal, IReadOnlyCollection<string>? roles, Func<string> message)
        {
        ...
        public int MaxConcurrentCommandsPerTopic
        {
        ...
        public int MaxConcurrentEventsPerTopic
        {
        ...
        public int? ReceiveCommandsBeforeExit
        {
        ...
        public void StartServices(ServiceSettings serviceSettings, IServiceCreator serviceCreator, IRelayRegister? relayRegister = null)
        {
            setupLock.Wait();
            try
            {
