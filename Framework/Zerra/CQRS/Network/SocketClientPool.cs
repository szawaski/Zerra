...
namespace Zerra.CQRS.Network
{
    internal class SocketClientPool : IDisposable
    {
        ...
        #if NETSTANDARD2_0
        public SocketPoolStream BeginStream(string host, int port, ProtocolType protocol, byte[] buffer, int offset, int count, bool requireNewConnection, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
...
#else
        public SocketPoolStream BeginStream(string host, int port, ProtocolType protocol, ReadOnlySpan<byte> buffer, bool requireNewConnection, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
