    ...
    internal sealed class TypeDetailRuntime<T> : TypeDetail<T>
    {
                                ...
                                if (hasInterfaces)
                                {
                                    ...
                                    foreach (var i in Interfaces)
                                    {
                                        ...
                                        foreach (var property in iProperties)
                                        {
                                            ...
                                            //    backingMember = MemberDetailRuntime<object, object>.New(Type, property.PropertyType, backingMemberName, backingField, null, false, locker);
                                            //}
        ...
        out MemberDetail member)
        {
            lock (locker)
            {
                ...
                var field = Type.GetField(name, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static | BindingFlags.NonPublic);
            ...
            lock (locker)
            {
                if (methodDetails is null)
                {
                    ...
                    if (!Type.IsGenericTypeDefinition)
                    {
                        ...
                        {
                            items.Add(new MethodDetailRuntime<T>(method.Name, method, false, locker));
                            if (hasInterfaces)
                        ...
                        if (hasInterfaces)
                        {
                            foreach (var i in Interfaces)
                            {
                                ...
                                foreach (var method in iMethods)
                                {
                                    ...
                                    if (!names!.Contains(name))
                                    {
                                        items.Add(new MethodDetailRuntime<T>(name, method, isExplicitFromInterface, locker));
                                        names!.Add(name);
                                    }
                                }
                            }
