    ...
    public sealed partial class PostgreSqlEngine : ITransactStoreEngine
    {
        ...
        private string GenerateSqlInsert<TModel>(TModel model, Graph<TModel>? graph, ModelDetail modelDetail, bool getIdentities) where TModel : class, new()
        {
            ...
            try
            {
                ...
                {
                    var isProperty = graph == null || graph.HasLocalProperty(modelPropertyDetail.Name);
        ...
        private string? GenerateSqlUpdate<TModel>(TModel model, Graph<TModel>? graph, ModelDetail modelDetail) where TModel : class, new()
        {
            ...
            try
            {
                ...
                {
                    if (graph == null || graph.HasLocalProperty(modelPropertyInfo.Name))
                    {
        ...
        private void AssureConstraints(bool create, bool update, bool delete, List<string> sql, ModelDetail model, SqlConstraint[] sqlConstraints)
        {
            ...
            foreach (var columnForRelation in model.RelatedNonEnumerableProperties)
            {
                ...
                if (columnForRelation.ForeignIdentity == null)
